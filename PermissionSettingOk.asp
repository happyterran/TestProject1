...
<head>
    ...
    <%If Err.Description <> "" Then%>
        <script language='javascript'>
            document.location.href="PermissionSetting.asp?MessageType=error&Message=사용자 설정이 불가능 합니다.    -    <%=Replace(Err.Description, "'", " ")%>"
        </script>
    <%Else%>
        <script language='javascript'>
            document.location.href="PermissionSetting.asp?MessageType=success&Message=정상적으로 사용자 수정이 완료 되었습니다"
        </script>
    <%End If%>
