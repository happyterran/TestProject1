...
function eXcell_clist(cell) {
    ...
    this.edit = function () {
        ...
        if (!a) return;
        var c = this.grid.clists2[this.cell._cellIndex];
        ...
        for (var i = 0; i < a.length; i++) {
            ...
            for (var j = 0; j < b.length; j++)
                if (a[i] == b[j]) fl = true;
            if (fl) text += "<div style='text-align:left'><input type='checkbox' checked='true' value='" + a[i] + "' /><label>" + c[i] + "</label></div>";
            else text += "<div style='text-align:left'><input type='checkbox' id='ch_lst_" + i + "' value='" + a[i] + "' /><label>" + c[i] + "</label></div>";
        };
        text += "<div><input type='button' value='Aplicar' style='width:100px;font-size:8pt;' onclick='this.parentNode.parentNode.editor.grid.editStop();'/></div>"
 ...
 this.detach = function (val) { if (this.obj) { this.setValue(this.obj.getValue());
 ...
 eXcell_clist.prototype.setValue = function (val) {
    if (typeof (val) == "object") { 
        ...
        if (optCol.length) this.cell._combo = [];
 ...
 dhtmlXGridObject.prototype.registerCList = function (col, list, list2) {
     if (!this.clists) {
         this.clists = new Array();
         this.clists2 = new Array();
     }
     if (typeof (list) != "object") list = list.split(",");
     this.clists[col] = list;
     
     if (typeof (list2) != "undefined")
         this.clists2[col] = list2;
    else
        this.clists2[col] = list;
 };
