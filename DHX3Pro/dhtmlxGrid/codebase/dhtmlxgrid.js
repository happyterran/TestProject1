...
function dhtmlXGridObject(a) {
	...
	this.setColumnSizes = function (e) {
		for (var a = 0, b = [], c = 0; c < this._cCount; c++) this.initCellWidth[c] == "*" && !this._hrrar[c] ? (this._awdth = !1, b.push(c)) : (this.cellWidthType == "%" ? (typeof this.cellWidthPC[c] == "undefined" && (this.cellWidthPC[c] = this.initCellWidth[c]), this.cellWidthPX[c] = Math.floor(e * this.cellWidthPC[c] / 100) || 0) : typeof this.cellWidthPX[c] == "undefined" && (this.cellWidthPX[c] = this.initCellWidth[c]), this._hrrar[c] || (a += this.cellWidthPX[c] * 1));
		if (b.length) {
			var d = Math.floor((e - a) / b.length);
			d < 0 && (d = 1);
			for (c = 0; c < b.length; c++) {
				var i = Math.max(this._drsclmW ? this._drsclmW[b[c]] || 0 : 0, d);
				this.cellWidthPX[b[c]] = i;
				a += i
			}
			if (e > a) {
				var l = b[b.length - 1];
	...
	this.setSizes = function () {
		if (this.hdr.rows[0]) {
			...
			if (this.globalBox) {
				...
				if (this._delta_x) this.entBox.parentNode.tagName == "TD" ? (this.entBox.style.width = "1px", this.entBox.style.width = parseInt(this._delta_x) * this.entBox.parentNode.clientWidth / 100 - a * 2 + "px") : this.entBox.style.width = this._delta_x;
			...
			if (!this.entBox.offsetWidth && (!this.globalBox || !this.globalBox.offsetWidth)) this._sizeTime = window.setTimeout(function () {
				...
				var d = (this.entBox.cmp || this._delta_x) && (this.skin_name || "").indexOf("dhx") == 0 && !e ? 2 : 0,
					i = (this.entBox.cmp || this._delta_y) && (this.skin_name || "").indexOf("dhx") == 0 && !e ? 2 : 0,
				...
				if (this._ahgr && (q = this._ahgrMA ? this.entBox.parentNode.clientHeight - s - t : this.obj.offsetHeight + (r ? m : 0) + (this._correction_y || 0), this._ahgrM && (q = this._ahgrF ? Math.min(this._ahgrM, q + s + t) - s - t : Math.min(this._ahgrM, q)), l && q >= this.obj.scrollHeight + (r ? m : 0))) l = !1, this.setColumnSizes(n - (this._correction_x || 0));
	...
	this.setInitWidths = function (a) {
		...
		if (_isFF) for (var b = 0; b < this.initCellWidth.length; b++) this.initCellWidth[b] != "*" && (this.initCellWidth[b] = parseInt(this.initCellWidth[b]))
	};
	...
	this.doColResize = function (a, b, c, f, d) {
		...
		var i = c + (a.clientX - f),
			l = d + (a.clientX - f);
		if (this.callEvent("onResize", [b._cellIndex, i, this])) {
			if (_isIE) this.objBox.scrollLeft = this.hdrBox.scrollLeft;
			if (b.colSpan > 1) {
				for (var m = [], k = 0; k < b.colSpan; k++) m[k] = Math.round(i * this.hdr.rows[0].childNodes[b._cellIndexS + k].offsetWidth / b.offsetWidth);
				for (k = 0; k < b.colSpan; k++) this._setColumnSizeR(b._cellIndexS + k * 1, m[k])
			} else this._setColumnSizeR(b._cellIndex, i);
			this.doOnScroll(0, 1);
			this.setSizes();
	...
	this.setSortImgState = function (a, b, c, f) {
		...
		if (convertStringToBoolean(a)) {
			...
			if (d) {
				for (var i = 0; i < d.childNodes.length; i++) if (d.childNodes[i]._cellIndexS == b) return this.r_fldSorted = d.childNodes[i], this.setSortImgPos();
	...
	this.setSortImgPos = function (a, b, c, f) {
		if (!this._hrrar || !this._hrrar[this.r_fldSorted ? this.r_fldSorted._cellIndex : a]) if (f || (f = a ? this.hdr.rows[c || 0].cells[a] : this.r_fldSorted), f != null) {
			var d = this.getPosition(f, this.hdrBox),
				i = f.offsetWidth;
			this.sortImg.style.left = Number(d[0] + i - 13) + "px";
			...
			if (this._ctmndx) {
				...
				if (this._ctmndx.showContextMenu) {
					var f = window.document.documentElement,
						...
						i = [f.scrollLeft || d.scrollLeft, f.scrollTop || d.scrollTop];
					if (_isIE) var l = a.clientX + i[0],
						m = a.clientY + i[1];
	...
	this.doClick = function (a, b, d, f) {
		...
		if (a.tagName == "TD") {
			if (this.checkEvent("onSelectStateChanged")) var i = this.getSelectedId();
			var l = this.row;
			if (d == 1) {
			...
			if (!r && !a.parentNode._sRow) if (this.callEvent("onBeforeSelect", [a.parentNode.idd, g])) {
				if (d == 0) {
			...
			if (this.checkEvent("onSelectStateChanged")) {
				...
				i != v && this.callEvent("onSelectStateChanged", [v, i])
			}
		}
		this.isActive = !0;
	...
	this.selectCell = function (a, b, c, f, d, i) {
		c || (c = !1);
		...
		l || (l = a.childNodes[0]);
		f ? this.doClick(l, c, 3, i) : this.doClick(l, c, 0, i);
		d && this.editCell()
	};
	this.moveToVisible = function (a, b) {
		...
		try {
			if (a.offsetHeight) {
				var f = a.offsetLeft + a.offsetWidth + 20,
					d = 0;
				...
				if (d && !b) this.objBox.scrollLeft = d
			}
			if (a.offsetHeight) f = a.offsetTop;
			else var i = this._realfake ? this._fake.rowsAr[a.parentNode.idd] : a.parentNode,
				f = this.rowsBuffer._dhx_find(i) * this._srdh;
			var l = f + a.offsetHeight + 38;
			if (l > this.objBox.offsetHeight + this.objBox.scrollTop) var m = l - this.objBox.offsetHeight;
			else f < this.objBox.scrollTop && (m = f - 5);
	...
	this._getNextCell = function (a, b, c) {
		var a = a || this.cell,
			f = a.parentNode;
		this._tabOrder ? (c = this._tabOrder[a._cellIndex], typeof c != "undefined" && (a = c < 0 ? this._nextRowCell(f, b, Math.abs(c) - 1) : f.childNodes[c])) : (c = a._cellIndex + b, c >= 0 && c < this._cCount ? (f._childIndexes && (c = f._childIndexes[a._cellIndex] + b), a = f.childNodes[c]) : a = this._nextRowCell(f, b, b == 1 ? 0 : this._cCount - 1));
	...
	this.selectRow = function (a, b, c, f) {
		typeof a != "object" && (a = this.render_row(a));
	...
	this.changeRowId = function (a, b) {
		if (a != b) {
			...
			if (this._mat_links && this._mat_links[a]) {
				...
				for (var i in g) for (d = 0; d < g[i].length; d++) this._compileSCL(g[i][d].original, g[i][d])
			}
	...
	this.setCellTextStyle = function (a, b, c) {
		...
		if (d) {
			...
			if (g) {
				var i = "";
				this._hrrar && this._hrrar[b] && (i = "display:none;");
				g.style.cssText = i + "width:" + g.style.width + ";" + c
			}
		}
	};
	...
	this.setColumnLabel = function (a, b, c, d) {
		var g = (d || this.hdr).rows[c || 1],
			i = g._childIndexes ? g._childIndexes[a] : a;
		if (g.cells[i]) {
			if (this.useImagesInHeader) {
				g.cells[i].style.textAlign = "left";
				g.cells[i].innerHTML = "<img src='" + this.imgURL + "" + b + "' onerror='this.src = \"" + this.imgURL + "imageloaderror.gif\"'>";
				var l = new Image;
				...
				k += b;
				k += "</div>";
				g.cells[i].innerHTML = k;
				if (this._hstyles[i]) g.cells[i].style.cssText = this._hstyles[i]
			}
			if ((b || "").indexOf("#") != -1) {
				var o = b.match(/(^|{)#([^}]+)(}|$)/);
				if (o) {
					var p = "_in_header_" + o[2];
					if (this[p]) this[p](this.forceDivInHeader ? g.cells[i].firstChild : g.cells[i], i, b.split(o[0]))
				}
			}
		}
	};
	...
	this.sortField = function (a, b, c) {
		...
		if (d && d.tagName == "TH" && this.fldSort.length - 1 >= d._cellIndex && this.fldSort[d._cellIndex] != "na") {
			var g = this.getSortingState(),
				i = g[0] == a && g[1] == "asc" ? "des" : "asc";
			if (this.callEvent("onBeforeSorting", [a, this.fldSort[a], i])) {
				this.sortImg.src = this.imgURL + "sort_" + (i == "asc" ? "asc" : "desc") + ".gif";
				if (this.useImagesInHeader) {
					...
					l.src = l.src.replace(/(\.[a-z]+)$/, "." + i + "$1")
				}
				this.sortRows(d._cellIndex, this.fldSort[d._cellIndex], i);
				this.fldSorted = d;
				this.r_fldSorted = c;
	...
	this.setHeader = function (a, b, c) {
		for (var d = typeof a != "object" ? this._eSplit(a) : [].concat(a), g = [], i = new dhtmlxArray(0), l = [], m = [], k = [], n = 0; n < d.length; n++) g[g.length] = Math.round(100 / d.length), i[i.length] = "ed", l[l.length] = "left", m[m.length] = "middle", k[k.length] = "na";
	...
	this.setStyle = function (a, b, c, d) {
		...
		for (var g = ["#" + this.entBox.id + " table.hdr td", "#" + this.entBox.id + " table.obj td", "#" + this.entBox.id + " table.obj tr.rowselected td.cellselected", "#" + this.entBox.id + " table.obj td.cellselected", "#" + this.entBox.id + " table.obj tr.rowselected td"], i = 0; !_isIE;) {
			try {
		...
		for (var k = 0; k < g.length; k++) this.ssModifier[k] && (_isIE ? document.styleSheets[0].addRule(g[k], this.ssModifier[k]) : document.styleSheets[i].insertRule(g[k] + (" { " + this.ssModifier[k] + " }"), document.styleSheets[i].cssRules.length))
	};
	...
	this.clearChangedState = function () {
		for (var a = 0; a < this.rowsCol.length; a++) for (var b = this.rowsCol[a], c = b.childNodes.length, d = 0; d < c; d++) b.childNodes[d].wasChanged = !1
	};
	this.getChangedRows = function (a) {
		...
		this.forEachRow(function (c) {
			...
			if (d.tagName == "TR") {
				...
				else for (var i = 0; i < g; i++) if (d.childNodes[i].wasChanged) {
					b[b.length] = d.idd;
					break
				}
			}
	...
	this.setSerializationLevel = function (a, b, c, d, g, i) {
		this._sUDa = a;
		this._sAll = b;
		this._sConfig = c;
		this._chAttr = d;
	...
	this._serialise = function () {
		...
		else for (var c = 0; c < this.rowsBuffer.length; c++) if (this.rowsBuffer[c]) {
	...
	this._serializeRow = function (a, b) {
		...
		if (this._sUDa && this.UserData[a.idd]) {
			...
			for (var i = 0; i < keysAr.length; i++) c.push("<userdata name='" + keysAr[i] + "'>" + (this._asCDATA ? "<![CDATA[" : "") + this.UserData[a.idd].get(keysAr[i]) + (this._asCDATA ? "]]\>" : "") + "</userdata>")
		}
		for (var l = !1, m = 0; m < this._cCount; m++) if (!this._srClmn || this._srClmn[m]) {
			var k = this.cells3(a, m);
			c.push("<cell");
			if (g.length) for (b = 0; b < g.length; b++) c.push(" " + g[b] + "='" + k.cell._attrs[g[b]] + "'");
			zxVal = k[this._agetm]();
			...
			this._sAll && this.cellType[m] == "tree" ? c.push((this._h2 ? " image='" + this._h2.get[a.idd].image + "'" : "") + ">" + zxVal + "</cell>") : c.push(">" + zxVal + "</cell>");
	...
	this._serialiseConfig = function () {
		for (var a = "<head>", b = 0; b < this.hdr.rows[0].cells.length; b++) if (!this._srClmn || this._srClmn[b]) {
			...
			if (d) for (var g = 0; g < d.keys.length; g++) a += "<option value='" + d.keys[g] + "'>" + d.values[g] + "</option>";
			a += "</column>"
		}
		return a += "</head>"
	};
	...
	this.getPosition = function (a, b) {
		...
		for (var b = b || document.body, d = a, g = 0, i = 0; d && d != b;) g += d.offsetLeft - d.scrollLeft, i += d.offsetTop - d.scrollTop, d = d.offsetParent;
		b == document.body && (_isIE ? (i += document.body.offsetTop || document.documentElement.offsetTop, g += document.body.offsetLeft || document.documentElement.offsetLeft) : _isFF || (g += document.body.offsetLeft, i += document.body.offsetTop));
		return [g, i]
	};
...
dhtmlXGridObject.prototype = {
		...
		if (d) {
			...
			if (b && d.style.display != "none") {
				...
				if (e != -1) {
					...
					for (var h = 0; h < d.childNodes.length; h++) d.childNodes[h].className = d.childNodes[h].className.replace(/cellselected/g, "");
	...
	setColumnHidden: function (a, b) {
		...
		if (this.fldSorted && this.fldSorted.cellIndex == a && b) this.sortImg.style.display = "none";
		var c = convertStringToBoolean(b);
		if (c) {
			if (this._hrrar) {
	...
	setColumnsVisibility: function (a) {
		if (a) this._ivizcol = a.split(this.delim);
	...
	_fixHiddenRowsAll: function (a, b, c, d, e) {
		for (var e = e || "_cellIndex", h = a.rows.length, j = 0; j < h; j++) {
			var f = a.rows[j].childNodes;
			if (f.length != this._cCount) for (var g = 0; g < f.length; g++) {
				if (f[g][e] == b) {
					f[g].style[c] = d;
					break
				}
	...
	_hideShowColumn: function (a, b) {
		var c = a;
		this.hdr.rows[1]._childIndexes && this.hdr.rows[1]._childIndexes[a] != a && (c = this.hdr.rows[1]._childIndexes[a]);
		if (b == "none") {
			...
			(_isOpera && _OperaRv < 9 || _isKHTML || _isFF) && this._fixHiddenRowsAll(this.hdr, a, "display", "none", "_cellIndexS");
			...
			(_isOpera || _isKHTML || _isFF) && this._fixHiddenRowsAll(this.hdr, a, "display", "", "_cellIndexS");
			...
			e._oldWidth && (this.cellWidthPX[a] = parseInt(e._oldWidth))
		}
		this.setSizes();
	...
	_aplNF: function (a, b) {
		...
		a[c[0]] = (a[c[0]] || "0") + c[4];
		if (c[1] > 0) for (var e = (c[0] > 0 ? 0 : 1) + c[0] + c[1]; e < a.length; e += c[1]) a[e] += c[5];
	...
	_parseHead: function (a) {
		...
		if (b.length) {
			...
			case "colwidth":
				...
				break;
			case "splitat":
				h = d[0].childNodes[j].firstChild ? d[0].childNodes[j].firstChild.data : !1
			}
			...
			if (c.length > 0) {
				...
				for (var f = [
					...
					[],
					[]
				], g = ",width,type,align,sort,color,format,hidden,id".split(","), i = ["", e, "setColTypes", "setColAlign", "setColSorting", "setColumnColor", "", "", "setColumnIds"], l = 0; l < c.length; l++) {
					for (var m = 1; m < g.length; m++) f[m].push(c[l].getAttribute(g[m]));
					f[0].push((c[l].firstChild ? c[l].firstChild.data : "").replace(/^\s*((\s\S)*.+)\s*$/gi, "$1"))
				}
				this.setHeader(f[0]);
				for (l = 0; l < i.length; l++) if (i[l]) this[i[l]](f[l].join(this.delim));
...
//####################################################################################################
	getUncheckedRows: function (col_ind) {
		var d = new Array();
		this.forEachRowA(function (id) {
			if (this.cells(id, col_ind).getValue() != 1) d.push(id)
		}, true)
		return d.join(",")
	},

	setCheckedRowsSetUpdated: function (cInd, v) {
		this.forEachRowA(function (id) {
			if (this.cells(id, cInd).getValue() != v) {
...
//####################################################################################################
		...
		if (this._ecspn) {
			...
			if (d._childIndexes && d.childNodes[d._childIndexes[b]]) {
				var e = d._childIndexes[b],
					...
					j = h.colSpan;
				h.colSpan = 1;
				if (j && j != 1) for (var f = 1; f < j; f++) {
					var g = document.createElement("TD");
					h.nextSibling ? d.insertBefore(g, h.nextSibling) : d.appendChild(g);
				...
				for (var i = b * 1 + 1 * j; i < d._childIndexes.length; i++) d._childIndexes[i] += (j - 1) * 1
			}
			if (c && c > 1) {
				if (d._childIndexes) e = d._childIndexes[b];
					...
					e = b;
					d._childIndexes = [];
					for (i = 0; i < d.childNodes.length; i++) d._childIndexes[i] = i
				}
				d.childNodes[e].colSpan = c;
				for (i = 1; i < c; i++) d._childIndexes[d.childNodes[e + 1]._cellIndex] = e, d.removeChild(d.childNodes[e + 1]);
				for (var l = d.childNodes[d._childIndexes[b]]._cellIndex, i = l * 1 + 1 * c; i < d._childIndexes.length; i++) d._childIndexes[i] -= c - 1
			}
		}
	},
	...
	adjustColumnSize: function (a, b) {
		if (!this._hrrar || !this._hrrar[a]) {
			...
			for (var h = this.obj.rows.length, j = 1; j < h; j++) {
	...
	_createHRow: function (a, b) {
		if (!b) {
			...
			this.ftr = b = d;
			for (var e = d.insertRow(0), h = this.hdrLabels.length <= 1 ? a[0].length : this.hdrLabels.length, j = 0; j < h; j++) e.appendChild(document.createElement("TH")), e.childNodes[j]._cellIndex = j;
		...
		for (j = 0; j < a[0].length; j++) if (a[0][j] == "#cspan") {
			...
			for (var i = b.rows.length - 2, l = !1, g = null; !l;) {
				for (var g = b.rows[i], m = 0; m < g.cells.length; m++) if (g.cells[m]._cellIndex == j) {
					l = m + 1;
					break
				}
				i--
			...
			k._cellIndex = k._cellIndexS = j;
			if (this._hrrar && this._hrrar[j] && !_isIE) k.style.display = "none";
			if (typeof a[0][j] == "object") k.appendChild(a[0][j]);
			else if (k.innerHTML = this.forceDivInHeader ? "<div class='hdrcell'>" + (a[0][j] || "&nbsp;") + "</div>" : a[0][j] || "&nbsp;", (a[0][j] || "").indexOf("#") != -1) if (d = a[0][j].match(/(^|{)#([^}]+)(}|$)/)) {
				var n = "_in_header_" + d[2];
				if (this[n]) this[n](this.forceDivInHeader ? k.firstChild : k, j, a[0][j].split(d[0]))
			}
			if (f) k.style.cssText = f[j];
			c.appendChild(k)
		}
		var o = b;
	...
	_refreshFromXML: function (a) {
		...
		for (var e = a.doXPath("//row"), h = 0; h < e.length; h++) {
			...
			if (this.rowsAr[f] && this.rowsAr[f].tagName != "TR") this._h2 ? this._h2.get[f].buff.data = j : this.rowsBuffer[this.getRowIndex(f)].data = j, this.rowsAr[f] = j;
				...
				this._refresh_mode[1] == "top" ? (this.rowsBuffer.unshift(g), i = 0) : this.rowsBuffer.push(g);
	...
	_process_xml_row: function (a, b) {
		...
		if (this._ud_enabled) for (var e = this.xmlLoader.doXPath("./userdata", b), h = e.length - 1; h >= 0; h--) {
			for (var j = "", f = 0; f < e[h].childNodes.length; f++) j += e[h].childNodes[f].nodeValue;
			this.setUserData(a.idd, e[h].getAttribute("name"), j)
		}
		for (f = 0; f < c.length; f++) {
			var g = c[this._c_order ? this._c_order[f] : f];
			if (g) {
				var i = a._childIndexes ? a._childIndexes[f] : f,
					l = g.getAttribute("type");
				if (a.childNodes[i]) {
					if (l) a.childNodes[i]._cellType = l;
					a.childNodes[i]._attrs = this._xml_attrs(g)
				}
	...
	_postRowProcessing: function (a, b) {
		...
		for (var c = 0, d = 0; d < a.childNodes.length; d++) {
			...
			if (j = e._attrs.align || this.cellAlign[h]) e.align = j;
			e.vAlign = e._attrs.valign || this.cellVAlign[h];
			var f = e._attrs.bgColor || this.columnColor[h];
			if (f) e.bgColor = f;
	...
	load: function (a, b, c) {
		...
		this._data_type = c;
		this.xmlLoader.onloadAction = function (a, e, h, j, f) {
	...
	_process_xml: function (a) {
		...
		if (c.tagName.toLowerCase() == this.xml.top) {
			...
			var d = a.doXPath(this.xml.row, c),
				...
				h = parseInt(a.doXPath("//" + this.xml.top)[0].getAttribute("total_count") || 0),
				j = !1;
			h && (this.rowsBuffer[h - 1] || (this.rowsBuffer.length && (j = !0), this.rowsBuffer[h - 1] = null), h < this.rowsBuffer.length && (this.rowsBuffer.splice(h, this.rowsBuffer.length - h), j = !0));
			if (this.isTreeGrid()) return this._process_tree_xml(a);
			for (var f = 0; f < d.length; f++) if (!this.rowsBuffer[f + e]) {
				var g = d[f].getAttribute("id") || f + e + 1;
				this.rowsBuffer[f + e] = {
					idd: g,
					data: d[f],
					_parser: this._process_xml_row,
	...
	sortRows: function (a, b, c) {
		...
		if (this.isTreeGrid()) this.sortTreeRows(a, b, c);
			...
			for (var j = 0; j < this.rowsBuffer.length; j++) d[this.rowsBuffer[j].idd] = this._get_cell_value(this.rowsBuffer[j], a, h);
	...
	_sortCore: function (a, b, c, d, e) {
		...
		if (b == "cus") {
			var j = this._customSorts[a];
			e[h](function (a, b) {
				return j(d[a.idd], d[b.idd], c, a.idd, b.idd)
			})
	...
	_addRow: function (a, b, c) {
		...
		if (this._fillers) {
			...
			this.rowsAr[d.idd] = d;
			for (var h = !1, j = 0; j < this._fillers.length; j++) {
				var f = this._fillers[j];
				if (f && f[0] <= c && f[0] + f[1] >= c) f[1] += 1, f[2].firstChild.style.height = parseInt(f[2].firstChild.style.height) + this._srdh + "px", h = !0, this._fake && this._fake._fillers[j][1]++;
...
(function () {
	...
	function e(a, b) {
		...
		for (var d = 1, e = [], h = 0; h < b.length; h++) if (typeof b[h] == "object" && b[h].length) for (var k = 0; k < b[h].length; k++) c(h, k, b[h][k]);
		else c(h, 0, b[h]);
...
dhtmlXGridObject.prototype._dp_init = function (a) {
	...
	a._getRowData = function (a) {
		...
		for (var d = this.obj.getRowById(a), e = 0; e < this.obj._cCount; e++) {
			var h = this.obj._c_order ? this.obj._c_order[e] : e,
				j = this.obj.cells(d.idd, e);
			if (!this._changed || j.wasChanged()) this._endnm ? c[this.obj.getColumnId(e)] = j.getValue() : c["c" + h] = j.getValue()
		}
		var f = this.obj.UserData[a];
		if (f) for (var g = 0; g < f.keys.length; g++) f.keys[g].indexOf("__") != 0 && (c[f.keys[g]] = f.values[g]);
